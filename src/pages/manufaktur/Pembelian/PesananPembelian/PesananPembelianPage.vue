    ...
    >
      <div class="col-12 col-md-8">
        <div class="text-h4 text-weight-bolder text-teal-10 leading-tight">
          ...
          <span class="text-h5 text-weight-light text-grey-6 block q-mt-xs"
            >Purchase Order Material & Kendali PO</span
          >
        </div>
        <div class="text-subtitle1 text-grey-7 q-mt-sm">
          Tinjau permintaan material dari lapangan dan terbitkan Purchase Order Material ke Supplier
          Material.
        </div>
      </div>
      <div class="col-12 col-md-auto q-mt-md q-mt-md-none text-right">
        ...
        <q-badge color="orange-9" class="q-px-md q-py-xs text-weight-bold shadow-2 rounded-12">
          {{ rows.filter((r) => r.status === 'Pending').length }} Permintaan Material
        </q-badge>
      </div>
    </div>
    ...
    <div class="q-mb-lg animate-fade no-print" v-if="poViewMode === 'list'">
      ...
      >
        <q-tab
          name="pr"
          icon="list_alt"
          label="Permintaan Material (PR)"
          class="rounded-12 q-px-lg text-weight-bold"
        />
        <q-tab
          name="po"
          icon="local_shipping"
          label="Purchase Order Material (PO)"
          class="rounded-12 q-px-lg text-weight-bold"
        />
      </q-tabs>
    </div>
            ...
            <div class="row items-center q-col-gutter-md">
              <div class="col-12 col-md-5">
                <q-input
                  ...
                  dense
                  rounded
                  placeholder="Cari No. permintaan, produksi, atau pemohon..."
                  bg-color="white"
                  color="teal-10"
                >
      ...
      <q-tab-panel name="po" class="q-pa-none">
        <div v-if="poViewMode === 'list'" class="animate-fade">
            ...
            <q-card-section class="q-py-md row items-center justify-between">
              ...
              <div class="col-12 col-sm-auto text-right">
                <q-btn
                  ...
                  rounded
                  no-caps
                  class="q-px-lg q-py-sm shadow-premium text-weight-bold btn-teal-main"
                  @click="openPoForm"
                />
              </div>
            </q-card-section>
          ...
          <q-card flat bordered class="rounded-20 shadow-sm bg-white" v-if="poRows.length === 0">
            <q-card-section class="text-center q-pa-xl text-grey-6">
              <q-icon
                ...
                color="teal-3"
              />
              <div class="text-h6 text-weight-bold text-teal-10">
                Data Purchase Order Material Belum Tersedia
              </div>
              <div class="text-subtitle2 q-mt-sm">
                Silakan klik "Buat PO Baru" untuk menerbitkan pesanan ke Supplier Material.
              </div>
            </q-card-section>
          </q-card>
          ...
          <q-card
            flat
            bordered
            class="rounded-20 shadow-sm overflow-hidden bg-white border-teal-thin"
            v-else
          >
            <q-table
              :rows="poRows"
                  ...
                  <q-td key="aksi" class="text-center" @click.stop>
                    <div class="row justify-center items-center q-gutter-xs no-wrap">
                      ...
                      <q-btn
                        ...
                        size="sm"
                        no-caps
                        class="q-px-sm text-weight-bold btn-teal-main"
    ...
    <div v-if="poViewMode === 'form'" class="animate-fade">
      <div class="row items-center justify-between q-mb-md">
        <div class="row items-center">
          ...
          />
          <div>
            <div class="text-h5 text-weight-black text-teal-10 uppercase letter-spacing-1">
              Penerbitan Purchase Order Material
            </div>
            <div class="text-subtitle1 text-grey-7">
              Isi formulir pengadaan barang ke Supplier Material
            </div>
          </div>
        </div>
        <q-btn
          ...
          :loading="submitting"
          @click="savePo"
          class="rounded-20 q-px-xl text-weight-bold shadow-4 btn-teal-main"
        />
      </div>
      ...
      <div class="row q-col-gutter-lg">
        ...
        <div class="col-12 col-md-5">
              ...
              <div class="q-gutter-y-md">
                <div>
                  <div class="label-req q-mb-xs text-primary">
                    Tarik Data Master Supplier Material
                  </div>
                  <q-select
                    outlined
                    dense
                    ...
                    :options="optSupplier"
                    option-label="nama"
                    :placeholder="poForm.supplier ? '' : 'Pilih Supplier Material...'"
                    bg-color="teal-50"
                    color="teal-10"
                    use-input
          ...
          <!-- Referensi Permintaan & Produksi -->
          <q-card flat bordered class="rounded-20 bg-white shadow-1 border-teal-thin">
            <q-card-section
              class="bg-teal-1 q-py-sm text-teal-10 text-weight-bold flex items-center border-bottom"
            >
            ...
            <q-card-section class="q-pa-lg">
              <div class="q-gutter-y-md">
                ...
                <div v-if="poForm.proyek_nama">
                  <div class="label-req q-mb-xs">Untuk Produksi / Gudang</div>
                  <q-input
                    outlined
                    dense
                ...
                <div v-if="poForm.no_spk">
                  <div class="label-req q-mb-xs">NO. REFF PRODUKSI</div>
                  <q-input
                    outlined
                    dense
        ...
        <div class="col-12 col-md-7">
          <q-card
            flat
            bordered
            class="rounded-20 q-mb-lg bg-white shadow-1 overflow-hidden border-teal-thin"
          >
            <q-toolbar class="bg-teal-10 text-white q-py-sm">
              ...
              <q-btn
                ...
                @click="addPoItem"
                no-caps
                class="text-weight-bold bg-teal-9 rounded-12 q-px-sm"
              />
            </q-toolbar>

            <q-markup-table flat separator="cell" class="po-entry-table">
              <thead class="bg-blue-grey-1 text-teal-10">
                <tr>
                  <th width="40">NO</th>
              ...
              <tbody>
                <tr v-for="(item, idx) in poForm.items" :key="idx">
                  ...
                  <td class="text-right text-weight-bold text-teal-10 bg-teal-50">
      ...
      <q-card class="column no-wrap bg-grey-4">
        <q-toolbar class="bg-white text-indigo-10 q-py-sm no-print shadow-2 shrink">
          ...
          <q-btn
            ...
            icon="send"
            label="Ajukan PO"
            class="text-weight-bold q-px-lg q-mr-sm shadow-3 btn-teal-main"
        ...
        <q-card-section class="col scroll q-pa-md q-pa-md-xl flex flex-center preview-container">
          <div id="po-print-area" class="po-print-container" v-if="selectedPo">
            ...
            >
              ...
              <div class="col-4">
                <div class="text-weight-bold q-mb-xl">Checked By</div>
                <div style="height: 60px"></div>
                <div class="text-weight-bold uppercase">
              ...
              <div class="col-4">
                <div class="text-weight-bold q-mb-xl">Approved By</div>
                <div style="height: 60px"></div>
                <div class="text-weight-bold uppercase">
...
import { useManufacturePurchaseStore } from 'src/stores/manufacturePurchaseStore'
import html2pdf from 'html2pdf.js'

// ── Init ──────────────────────────────────────────────────────────────────
const $q = useQuasar()
const authStore = useAuthStore()
const purchaseStore = useManufacturePurchaseStore()
...
// State PR
const rows = computed(() => purchaseStore.materialRequests)
...
// State PO
const filterPo = ref('')
const poRows = computed(() => purchaseStore.purchaseOrders)
const showPoPreview = ref(false)
const selectedPo = ref(null)
const submitting = ref(false)
const tempLogoPo = ref(null)
const optSupplier = ref([])
const allSupplier = computed(() => purchaseStore.suppliers)
...
const poFormDefault = {
  ...
  closing:
    'Kami berharap Purchase Order Material ini dapat memenuhi kebutuhan Kami. Jika ada pertanyaan atau klarifikasi lebih lanjut, jangan ragu untuk menghubungi kami.\nTerima kasih atas perhatiannya.',
  prepared_by: '',
  checked_by: '',
  approved_by: '',
}
...
const config = computed(() => purchaseStore.config)

// ── Columns ───────────────────────────────────────────────────────────────
const columns = [
  { name: 'nomor', align: 'left', label: 'NO. PERMINTAAN', field: 'nomor', sortable: true },
  { name: 'proyek_nama', align: 'left', label: 'PRODUKSI / PEMOHON', field: 'proyek_nama' },
  {
    name: 'total_estimasi',
    align: 'right',
    label: 'AMOUNT (IDR)',
...
const fetchData = async () => {
  loading.value = true
  try {
    await purchaseStore.loadConfig()
    await purchaseStore.loadSuppliers()
    optSupplier.value = [...purchaseStore.suppliers]
    purchaseStore.bindMaterialRequests()
    purchaseStore.bindPurchaseOrders()
  } catch (error) {
    console.error(error)
    purchaseStore.materialRequests = []
    purchaseStore.purchaseOrders = []
...
const openPoForm = async () => {
  $q.loading.show()
  try {
    const nextNum = (await purchaseStore.countPurchaseOrders()) + 1
...
const handleLogoUploadPo = async (file) => {
  ...
  reader.onload = async () => {
    try {
      ...
      await purchaseStore.saveLogo(reader.result)
...
const onPrSelect = (prData) => {
  if (prData) {
    const projectId = prData.project_id || prData.projectId || prData.proyek_id || ''
    const projectName = prData.project_name || prData.projectName || prData.proyek_nama || prData.gudang_nama || ''
    poForm.value.project_id = projectId
    poForm.value.project_name = projectName
    poForm.value.proyek_id = projectId
    poForm.value.proyek_nama = projectName
    ...
    poForm.value.items = prData.items.map((it) => ({
      ...
      project_name: it.project_name || projectName,
      proyek_id: it.proyek_id || projectId,
      proyek_nama: it.proyek_nama || projectName,
    }))
  } else {
    poForm.value.project_id = ''
    poForm.value.project_name = ''
    poForm.value.proyek_id = ''
...
const savePo = async () => {
  ...
  try {
    ...
    await purchaseStore.createPurchaseOrder(payload, poForm.value.items)

    if (poForm.value.referensi_pr?.id) {
      await purchaseStore.updateMaterialRequest(poForm.value.referensi_pr.id, { status: 'Ordered' })
    }

    $q.notify({ type: 'positive', message: 'Purchase Order Material berhasil diterbitkan!' })
...
const ajukanPo = (row) => {
  $q.dialog({
    title: 'Ajukan Purchase Order Material?',
    ...
    try {
      await purchaseStore.updatePurchaseOrder(row.id, {
        status: 'Submitted',
...
.btn-teal-main:hover {
  box-shadow: 0 8px 24px rgba(0, 150, 136, 0.45) !important;
