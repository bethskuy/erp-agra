  ...
  <q-page class="bg-grey-2 q-pa-md q-pa-md-lg font-pro">
      ...
      <q-card-section class="q-py-md">
        <div class="row items-center q-col-gutter-md">
          ...
          <div class="col-12 col-md-auto text-caption text-grey-6">
            Total Katalog:
            <span class="text-weight-bold text-teal-10">{{ rows.length }} Item Terdaftar</span>
          </div>
        </div>
      </q-card-section>
    </q-card>
        ...
        <template v-slot:body="props">
          ...
          >
            ...
            <q-td key="foto" class="text-center">
              <q-avatar size="40px" rounded color="grey-2" class="shadow-sm border-teal-thin">
                <q-img
                  :src="
                    props.row.foto_url ||
    ...
    >
      <q-card class="bg-grey-2 column no-wrap print-fixed-card" v-if="selectedItem">
        <q-toolbar class="bg-teal-10 text-white q-py-md shadow-2 shrink no-print">
          ...
          <div id="material-detail-print" class="row justify-center print-container">
            <div class="col-12 col-lg-10">
              ...
              <div class="print-only q-mb-xl text-center">
                ...
                <div
                  class="q-mt-md"
                  style="height: 2px; background: #004d40; width: 100px; margin: 15px auto"
                ></div>
              </div>
              ...
              <div class="row q-col-gutter-xl print-row-fix">
                ...
                <div class="col-12 col-md-4">
                  <q-card
                    flat
                    bordered
                    class="rounded-20 overflow-hidden shadow-premium bg-white border-teal-thin"
                  >
                    <q-img
                      :src="
                      ...
                      class="bg-grey-1"
                    />
                    <q-card-section class="bg-teal-10 text-white text-center print-teal-bg">
                  ...
                  <q-card flat bordered class="rounded-20 q-mt-lg bg-white shadow-1">
                    <q-card-section>
                      ...
                      <div class="row items-center q-mt-sm">
                        ...
                        <div class="text-subtitle1 text-weight-bold uppercase text-teal-10">
                          {{ selectedItem.kategori }}
                        </div>
                      </div>
                      <q-separator class="q-my-md" />
                      <div class="text-overline text-grey-6 uppercase">Satuan Ukur</div>
                      <div class="row items-center q-mt-sm text-teal-10 text-weight-bolder">
                        <q-icon name="straighten" color="teal-10" size="sm" class="q-mr-sm" />
                ...
                <div class="col-12 col-md-8">
                  <div
                    class="text-h3 text-weight-black text-teal-10 text-uppercase leading-tight q-mb-xs print-text-teal"
                  >
                    {{ selectedItem.nama }}
                  </div>
                    ...
                    <q-markup-table flat separator="horizontal" class="detail-vendor-table">
                      ...
                      <tbody>
                        <tr v-for="(v, i) in selectedItem.vendor_prices" :key="i">
                          <td class="text-left">
                            <div class="text-weight-bold text-teal-9 text-uppercase">
                  ...
                  <div class="q-mt-lg row q-col-gutter-md">
                    <div class="col-12">
                      <q-card
                        flat
                        bordered
                        class="rounded-20 bg-teal-1 text-teal-10 shadow-sm border-teal-thin"
                      >
                      ...
                      <div class="row justify-center q-mb-lg">
                        <div class="column items-center">
                          <q-avatar
                            ...
                            rounded
                            color="grey-1"
                            class="shadow-2 border-teal-thin q-mb-sm overflow-hidden"
                          >
                            <q-img
                          ...
                          >
                            <template v-slot:prepend
                              ><q-icon name="photo_camera" color="teal-10"
                            /></template>
                          </q-file>
                        </div>
                      </div>
                ...
                <div class="col-12 col-md-7">
                  <q-card flat bordered class="rounded-20 q-pa-lg bg-white shadow-1 h-full">
                    <div class="row items-center justify-between q-mb-lg">
                      <div
                        class="text-subtitle1 text-teal-10 text-weight-bolder flex items-center"
                      >
                        <q-icon name="payments" class="q-mr-sm" /> PERBANDINGAN HARGA VENDOR
                      </div>
                      <q-btn
                      ...
                      <div class="q-gutter-y-sm">
                        ...
                        <div class="row q-col-gutter-sm items-center">
                          ...
                          <div class="col-12 col-sm-5">
                            ...
                            >
                              <template v-slot:prepend
                                ><q-icon name="cloud_upload" size="xs" color="teal-7"
                              /></template>
                              <template
                                v-slot:append
...
const canAction = (actionType) => {
  ...
  const targetId = '_manufaktur_master_barang-list'
...
const fetchReferences = async () => {
  try {
    const [satSnap, catSnap, venSnap] = await Promise.all([
      getDocs(query(collection(db, 'manufactur_master_satuan'), orderBy('nama', 'asc'))),
      getDocs(query(collection(db, 'manufactur_master_kategori_barang'), orderBy('nama', 'asc'))),
      getDocs(query(collection(db, 'manufactur_master_supplier'), orderBy('nama', 'asc'))),
    ])
...
const quickAddSatuan = () => {
  $q.dialog({
    ...
    if (!data) return
    await addDoc(collection(db, 'manufactur_master_satuan'), { nama: data, createdAt: serverTimestamp() })
...
const quickAddKategori = () => {
  $q.dialog({
    ...
    if (!data) return
    await addDoc(collection(db, 'manufactur_master_kategori_barang'), { nama: data, createdAt: serverTimestamp() })
    await fetchReferences()
...
const fetchBarang = () => {
  loading.value = true
  const q = query(collection(db, 'manufactur_master_barang'), orderBy('nama', 'asc'))
...
const simpanBarang = async () => {
  ...
  try {
    if (itemPhotoFile.value && !form.value.foto_base64) {
      const pRef = storageRef(storage, `manufaktur/master/barang/photos/${Date.now()}_ITEM`)
    ...
    for (let vp of form.value.vendor_prices) {
      if (vp.docFile && !vp.quotation_base64) {
        const sRef = storageRef(storage, `manufaktur/master/barang/quotations/${Date.now()}_QUOT`)
    ...
    if (isEditMode.value) {
      const docRef = doc(db, 'manufactur_master_barang', form.value.id)
      delete payload.id
      ...
      await addDoc(collection(db, 'manufactur_master_barang'), payload)
    }
...
const hapusBarang = (data) => {
  $q.dialog({
    ...
    await deleteDoc(doc(db, 'manufactur_master_barang', data.id))
