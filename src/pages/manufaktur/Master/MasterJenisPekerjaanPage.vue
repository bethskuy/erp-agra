<template>
  ...
  >
        ...
        <div class="form-page-header row items-start justify-between q-mb-lg">
          <div class="col">
            <q-btn
              ...
              color="green-10"
              icon="arrow_back"
              label="Kembali ke Daftar Jenis Pekerjaan"
              no-caps
              class="q-mb-md"
              @click="closeFormPage"
            />
            ...
            <div class="text-subtitle1 text-grey-7 q-mt-xs form-page-subtitle">
              Kelola klasifikasi pekerjaan fabrikasi untuk pengelompokan proyek workshop.
            </div>
          </div>
          <div class="col-12 col-sm-auto q-mt-md q-mt-sm-none">
            <q-chip color="green-10" text-color="white" icon="work_outline" class="text-weight-bold">
        ...
        <q-form class="enterprise-form-shell" @submit.prevent="saveRow">
          <q-card flat bordered class="enterprise-form-card">
            <q-card-section class="form-section-header">
              <div class="text-h6 text-weight-bold text-green-10">Informasi Jenis Pekerjaan</div>
              <div class="text-caption text-grey-7">
                Data ini menjadi referensi kategori pekerjaan pada SPK Produksi dan fabrikasi proyek.
              </div>
            </q-card-section>
            <q-separator />
    ...
    <div class="page-header row items-center justify-between q-mb-lg">
      <div>
        <div class="text-h4 text-weight-bolder text-green-10">Master Jenis Pekerjaan</div>
        <div class="text-subtitle1 text-grey-7 q-mt-xs">
          Klasifikasi pekerjaan fabrikasi untuk pengelompokan proyek workshop.
        </div>
      </div>
        ...
        <div class="row q-col-gutter-md items-center">
          <div class="col-12 col-md-7">
            <q-input
              ...
              rounded
              debounce="250"
              placeholder="Cari kode jenis, nama jenis, atau deskripsi..."
            >
              <template #prepend>
    ...
    <q-card flat bordered class="table-card">
      ...
      >
        ...
        <template #body-cell-nama_jenis="props">
          <q-td :props="props">
            <div class="text-weight-bold text-green-10">{{ props.row.nama_jenis || '-' }}</div>
            <div class="text-caption text-grey-6">{{ props.row.kode_jenis || '-' }}</div>
          </q-td>
        </template>
        ...
        <template #body-cell-aksi="props">
          <q-td :props="props" @click.stop>
            <q-btn flat round dense color="green-10" icon="edit" @click="openFormPage(props.row)">
              <q-tooltip>Edit jenis pekerjaan</q-tooltip>
            </q-btn>
            <q-btn flat round dense color="negative" icon="delete" @click="confirmDelete(props.row)">
              <q-tooltip>Hapus jenis pekerjaan</q-tooltip>
            </q-btn>
          </q-td>
        </template>
        ...
        <template #no-data>
          <div class="full-width row flex-center text-grey-7 q-pa-xl">
            <q-icon name="work_outline" size="28px" class="q-mr-sm" />
...
const COLLECTION_NAME = 'master_jenis_pekerjaan'
...
const columns = [
  { name: 'nama_jenis', label: 'Jenis Pekerjaan', field: 'nama_jenis', align: 'left', sortable: true },
...
const formPageTitle = computed(() =>
  selectedId.value ? 'Edit Jenis Pekerjaan' : 'Tambah Jenis Pekerjaan',
)
...
const filteredRows = computed(() => {
  ...
  return rows.value.filter((row) => {
    ...
    const matchesSearch =
      !term ||
      [row.kode_jenis, row.nama_jenis, row.deskripsi]
        .join(' ')
        .toLowerCase()
        .includes(term)
...
const payloadFromForm = () => ({
  kode_jenis: form.value.kode_jenis,
  nama_jenis: form.value.nama_jenis,
...
const saveRow = async () => {
  ...
  try {
    ...
    if (selectedId.value) {
      ...
      notify('positive', 'Jenis pekerjaan berhasil diperbarui.')
    } else {
      ...
      notify('positive', 'Jenis pekerjaan berhasil ditambahkan.')
    }
    closeFormPage()
  } catch (error) {
    console.error(error)
    notify('negative', 'Gagal menyimpan jenis pekerjaan.')
  } finally {
    saving.value = false
  }
}
...
const confirmDelete = (row) => {
  $q.dialog({
    ...
    message: `Hapus jenis pekerjaan ${row.nama_jenis || row.kode_jenis}?`,
    cancel: true,
    persistent: true,
    ...
    try {
      ...
      notify('positive', 'Jenis pekerjaan berhasil dihapus.')
    } catch (error) {
      console.error(error)
      notify('negative', 'Gagal menghapus jenis pekerjaan.')
    }
  })
}
...
onMounted(() => {
  ...
  unsubscribeRows = onSnapshot(
    ...
    (error) => {
      console.error(error)
      loading.value = false
      notify('negative', 'Gagal memuat data jenis pekerjaan.')
    },
  )
})
