  ...
  <q-page class="bg-grey-2 q-pa-md q-pa-md-lg font-pro">
      ...
      <q-card-section class="q-py-md">
        <div class="row items-center q-col-gutter-md">
          ...
          <div class="col-12 col-md-auto text-caption text-grey-6">
            Total Grup:
            <span class="text-weight-bold text-teal-10">{{ rows.length }} Kategori</span>
          </div>
        </div>
      </q-card-section>
    </q-card>
    ...
    <q-dialog v-model="showDetail" maximized transition-show="fade" transition-hide="fade">
      <q-card class="bg-grey-2 column no-wrap">
        <q-toolbar class="bg-teal-10 text-white q-py-md shadow-2 shrink">
            ...
            <div class="col-12 col-md-10 col-lg-8">
              ...
              >
                <div class="row items-center">
                  <div
                    class="col-12 col-md-4 bg-teal-1 flex flex-center q-pa-xl"
                  ...
                  <div class="col-12 col-md-8 q-pa-xl">
                    ...
                    <div class="text-h3 text-weight-black text-teal-10 q-mb-xs uppercase">
                ...
                <div class="q-gutter-y-lg">
                  <div>
                    ...
                    <q-input
                      outlined
                      v-model="form.nama"
                      placeholder="Contoh: manufaktur Sipil, Infrastruktur..."
                      bg-color="white"
                      autofocus
...
const canAction = (actionType) => {
  ...
  const targetId = '_manufaktur_master_proyek-kategori'
...
const fetchKategori = () => {
  loading.value = true
  const q = query(collection(db, 'manufactur_master_kategori_proyek'), orderBy('nama', 'asc'))
...
const simpanKategori = async () => {
  ...
  try {
    ...
    if (isEditMode.value) {
      await updateDoc(doc(db, 'manufactur_master_kategori_proyek', form.value.id), payload)
    } else {
      payload.createdAt = serverTimestamp()
      await addDoc(collection(db, 'manufactur_master_kategori_proyek'), payload)
    }
...
const hapusKategori = (data) => {
  $q.dialog({
    ...
    try {
      await deleteDoc(doc(db, 'manufactur_master_kategori_proyek', data.id))
