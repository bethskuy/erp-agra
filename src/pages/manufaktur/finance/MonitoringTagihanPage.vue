...
const resolvePermission = (karyawanData) => {
  ...
  const menuTagihan = modulManufaktur.menus?.find((menu) => {
...
const fetchData = () => {
  ...
  unsubInvoiceCust = onSnapshot(collection(db, 'finance_invoice_customer_manufactur'), (snap) => {
...
const simpanTagihan = async () => {
  ...
  try {
    ...
    if (isEditTagihanMode.value) {
      ...
      await updateDoc(doc(db, 'monitoring_tagihan_spk_manufactur', formTagihan.value.id), payload)
      $q.notify({ type: 'positive', position: 'top', message: 'Tagihan berhasil diperbarui' })
    } else {
      payload.createdAt = serverTimestamp()
      await addDoc(collection(db, 'monitoring_tagihan_spk_manufactur'), payload)
...
const deleteTagihan = (row) => {
  ...
  $q.dialog({
    ...
    try {
      await deleteDoc(doc(db, 'monitoring_tagihan_spk_manufactur', row.id))
...
const savePayment = async () => {
  ...
  try {
    ...
    await updateDoc(doc(db, 'monitoring_tagihan_spk_manufactur', selectedTagihan.value.id), {
