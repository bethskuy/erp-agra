...
const canAction = (actionType) => {
  ...
  const menu = modulePerm.menus.find((m) => {
    ...
    return (
      (idLower.endsWith('_invoice') || idLower === '_manufaktur_invoice') &&
  ...
  if (val) {
    ...
    try {
      const spkSnap = await getDocs(
        query(collection(db, 'manufactur_master_proyek_spk'), where('projectId', '==', val.id)),
      )
      if (!spkSnap.empty) {
...
const openPreviewDialog = async (row) => {
  ...
  if (
    ...
    try {
      await updateDoc(doc(db, 'finance_invoice_customer_manufactur', row.id), { creator_read: true })
...
const simpanInvoice = async () => {
  ...
  try {
    ...
    if (isEditMode.value && currentInvoiceId.value) {
      ...
      await updateDoc(doc(db, 'finance_invoice_customer_manufactur', currentInvoiceId.value), payload)
    } else {
      payload.createdAt = serverTimestamp()
      payload.approval_status = 'Pending'
      payload.status = 'Draft'
      await addDoc(collection(db, 'finance_invoice_customer_manufactur'), payload)
    }
...
const confirmHapus = (row) => {
  ...
  $q.dialog({
    ...
    await deleteDoc(doc(db, 'finance_invoice_customer_manufactur', row.id))
