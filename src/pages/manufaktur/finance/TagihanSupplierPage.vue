...
const resolvePermission = (karyawanData) => {
  ...
  if (!detail || !Array.isArray(detail)) {
    const hasManufaktur = karyawanData?.akses?.includes('manufaktur') || false
  ...
  const modulManufaktur = detail.find((m) => m.id === 'manufaktur')
  if (!modulManufaktur || !modulManufaktur.isActive) {
    userPermission.value = { lihat: false, buat: false, ubah: false, hapus: false, approve: false }
    return
  }

  const menuTagihanSupplier = modulManufaktur.menus?.find((menu) => {
...
const loadUserPermission = () => {
  ...
  onAuthStateChanged(auth, async (user) => {
    ...
    try {
      const q = query(collection(db, 'karyawan_manufaktur'), where('email', '==', user.email))
      const snap = await getDocs(q)

      if (!snap.empty) {
        const karyawanData = snap.docs[0].data()
        resolvePermission(karyawanData)
        onSnapshot(doc(db, 'karyawan_manufaktur', snap.docs[0].id), (docSnap) => {
...
const fetchData = async () => {
  ...
  const snapSupp = await getDocs(collection(db, 'manufactur_master_supplier'))
...
const processHybridUpload = async (file, pathName) => {
  ...
  if (file.size <= 512000) {
    ...
    const sRef = storageRef(storage, `finance_manufactur/tagihan/${Date.now()}_${pathName}`)
...
const simpanTagihan = async () => {
  ...
  try {
    ...
    if (isEditMode.value) {
      await updateDoc(doc(db, 'finance_tagihan_manufactur', form.value.id), payload)
    } else {
      payload.createdAt = serverTimestamp()
      payload.pembuat = authStore.user?.nama || 'Admin'
      await addDoc(collection(db, 'finance_tagihan_manufactur'), payload)
    }
...
const confirmHapus = (row) => {
  ...
  $q.dialog({
    ...
    await deleteDoc(doc(db, 'finance_tagihan_manufactur', row.id))
...
const savePayment = async () => {
  ...
  try {
    if (paymentForm.value.bukti_file) {
      ...
      const fRef = storageRef(storage, `finance_manufactur/pembayaran_tagihan/${Date.now()}_${file.name}`)
    ...
    await updateDoc(doc(db, 'finance_tagihan_manufactur', selectedTagihan.value.id), {
