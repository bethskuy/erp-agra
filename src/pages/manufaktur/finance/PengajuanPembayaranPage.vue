...
const fetchData = async () => {
  ...
  const qTagihan = query(
    collection(db, 'finance_tagihan_manufactur'),
...
const processHybridUpload = async (file, pathName) => {
  ...
  if (file.size <= 512000) {
    ...
    const sRef = storageRef(storage, `finance_manufactur/pengajuan/${Date.now()}_${pathName}`)
...
const simpanPengajuan = async () => {
  ...
  try {
    ...
    if (isEditMode.value) {
      ...
      await updateDoc(doc(db, 'finance_pengajuan_pembayaran_manufactur', form.value.id), payload)
    } else {
      ...
      await addDoc(collection(db, 'finance_pengajuan_pembayaran_manufactur'), payload)
    }
...
const confirmHapus = (row) => {
  ...
  $q.dialog({
    ...
    await deleteDoc(doc(db, 'finance_pengajuan_pembayaran_manufactur', row.id))
    $q.notify({
      type: 'positive',
      position: 'top',
      icon: 'delete_forever',
...
onMounted(() => {
  ...
  if (email) {
    const qUser = query(collection(db, 'karyawan_manufaktur'), where('email', '==', email))
