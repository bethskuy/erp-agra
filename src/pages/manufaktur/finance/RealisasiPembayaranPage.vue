        ...
        <table class="report-table" style="width: 100%; border-collapse: collapse">
          <thead>
            <tr>
              ...
              <th style="width: 25%; text-align: left">TGL CAIR / TARGET</th>
...
const processRealisasi = async () => {
  ...
  try {
    ...
    const fRef = storageRef(storage, `finance_manufactur/bukti_transfer/${Date.now()}_${file.name}`)
    ...
    await updateDoc(doc(db, 'finance_pengajuan_pembayaran_manufactur', selectedData.value.id), updateData)

    if (selectedData.value.tagihan_id) {
      await updateDoc(doc(db, 'finance_tagihan_manufactur', selectedData.value.tagihan_id), {
        status: 'Lunas',
