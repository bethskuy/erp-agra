<template>
  <q-page class="warehouse-page bg-dark-premium q-pa-md q-pa-lg-lg">
    <div class="row items-center justify-between q-mb-lg">
      <div>
        <div class="text-h4 text-weight-bolder incoming-title">Bahan Jadi</div>
        <div class="text-subtitle1 incoming-subtitle">
          Monitoring stok bahan jadi warehouse manufaktur.
        </div>
      </div>
      <q-btn
        flat
...
const COLLECTION_NAME = 'bahan_jadi_manufaktur'
const $q = useQuasar()
const rows = ref([])
...
const loadRows = () => {
  ...
  unsubscribeRows = onSnapshot(
    ...
    (error) => {
      console.error(error)
      loading.value = false
      $q.notify({ type: 'negative', message: 'Gagal memuat bahan jadi' })
    },
  )
}
