<template>
  <q-page class="bg-grey-2 q-pa-md q-pa-md-lg font-pro">
    ...
    <div class="row items-center justify-between q-mb-xl animate-fade no-print">
      ...
      <!-- FITUR BARU: NOTIFIKASI REALTIME GABUNG SINI[cite: 7] -->
      <div class="col-12 col-md-auto q-mt-md q-mt-md-none text-right relative-position">
        ...
        <!-- Animasi +N saat ada data baru masuk -->
        <transition
          appear
        ...
        >
          <q-badge
            ...
            class="text-weight-bold shadow-5 q-pa-xs"
    ...
    <q-card flat bordered class="q-mb-lg shadow-1 rounded-20 bg-white no-print">
      <q-card-section class="q-py-md">
        <div class="row items-center q-col-gutter-md">
          ...
          <q-space />
          <q-btn flat round icon="refresh" color="teal" @click="fetchApprovalData" />
        </div>
      </q-card-section>
    </q-card>
      ...
      >
        <template v-slot:header="props">
          <q-tr :props="props" class="bg-teal-10 text-white">
            <!-- FIX: Tag penutup sudah benar q-th[cite: 7] -->
        ...
        <q-toolbar class="bg-white text-teal-10 q-py-sm no-print shadow-2 shrink">
          ...
          >
            <q-list class="q-pa-sm" style="min-width: 220px">
              ...
              >
                <q-item-section avatar><q-icon name="gesture" color="teal" /></q-item-section>
              ...
              <q-item clickable v-ripple class="rounded-borders relative-position">
                <q-item-section avatar><q-icon name="upload" color="teal" /></q-item-section>
          ...
          <div id="quotation-print" class="letter-paper shadow-24" v-if="selectedData">
            ...
            <div class="row justify-between items-start q-mt-lg q-mb-md text-left">
              ...
              <div class="col-5 text-right">
                <div class="quotation-title-pro uppercase text-teal-10">Quotation</div>
                <div class="quotation-no-pro text-teal-10">{{ selectedData.nomor }}</div>
            ...
            <div class="terms-container border-teal-10 text-left q-mt-lg">
              <div class="terms-header bg-teal-10 uppercase">Syarat & Kondisi Pembayaran :</div>
            ...
            <div class="signature-container text-left q-mt-xl">
              ...
              <div class="row q-mt-md">
                ...
                <div class="col-12 col-sm-7 text-right">
                  ...
                  <div
                    class="text-signer-final text-weight-bolder underline uppercase text-teal-10 border-teal-bottom"
                  >
    ...
    <q-dialog v-model="showPad" persistent backdrop-filter="blur(4px)">
      <q-card style="width: 500px; max-width: 95vw" class="rounded-20 shadow-24">
        <q-card-section class="row items-center q-pb-none bg-teal-10 text-white q-pa-md">
          <div class="text-h6 text-weight-bold uppercase">Gurat Tanda Tangan</div>
          <q-space />
          <q-btn icon="close" flat round dense v-close-popup />
        </q-card-section>
        <q-card-section class="q-pa-lg">
          <div class="signature-pad-wrapper border-teal-dashed shadow-inner">
...
// STATE BARU UNTUK NOTIFIKASI REALTIME[cite: 7]
...
// FETCH DATA DENGAN REALTIME LISTENER + NOTIFIKASI[cite: 7]
const fetchApprovalData = () => {
  ...
  try {
    ...
    unsubApproval = onSnapshot(q, (snap) => {
      ...
      // Hitung jumlah pending terbaru[cite: 7]
      const currentPendingSize = allData.filter((r) => r.status === 'Pending').length

      // Jika ada penambahan data baru, munculkan animasi +N[cite: 7]
      if (currentPendingSize > pendingCount.value && pendingCount.value !== 0) {
        lastAddedCount.value = currentPendingSize - pendingCount.value
      ...
      pendingCount.value = currentPendingSize
      loading.value = false
    })
  } catch (e) {
    console.error(e)
...
const handleApproval = (row, status, alasan = null) => {
  $q.dialog({
    ...
    try {
      $q.loading.show()
      const data = { status: status, updatedAt: serverTimestamp(), processedAt: serverTimestamp() }
...
const promptReject = (row) => {
  $q.dialog({
    ...
    prompt: {
      model: '',
      type: 'textarea',
      placeholder: 'Contoh: Harga material terlalu tinggi...',
    },
    cancel: true,
...
/* CSS BARU UNTUK ANIMASI BADGE[cite: 7] */
.animated {
...
.hover-bg:hover {
  background-color: rgba(0, 77, 64, 0.03) !important;
}
.transition-all {
