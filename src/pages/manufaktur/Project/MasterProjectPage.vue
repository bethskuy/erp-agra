          ...
          <div class="row items-center justify-between q-mb-xl">
            <div class="col-12 col-sm-8">
              ...
              <div class="text-subtitle1 text-grey-7 q-mt-sm">
                Monitoring seluruh portofolio project manufaktur PT AGRA secara terpusat.
              </div>
            </div>
          </div>
        ...
        >
          ...
          <div class="row items-center justify-between q-mb-lg q-col-gutter-sm">
            <div class="col-12 col-md-auto row items-center">
              ...
              <div>
                <div
                  class="text-h5 text-weight-black text-brand-primary uppercase letter-spacing-1"
                >
                  Data Teknis Kontrak
                </div>
            ...
            <q-tab-panels v-model="activeTab" animated class="bg-grey-1">
              ...
              <q-tab-panel name="boq" class="q-pa-lg">
                ...
                <div class="q-mt-xl">
                  ...
                  >
                    <div
                      class="text-overline opacity-80 uppercase tracking-widest text-weight-bold"
                    >
                      Grand Total Kontrak
                    </div>
                  ...
                  >
                        ...
                        <div class="col-12 col-sm-auto row items-center q-gutter-xs no-wrap">
                          ...
                          >
                            <template v-slot:append>
                              <span class="text-caption text-brand-primary text-weight-bold"
                                >%</span
                              >
                            </template>
                          </q-input>
                    ...
                    <div v-else class="q-pa-lg text-center text-grey-5">
                      ...
                      <div class="text-caption">
                        Perhitungan PPN dinonaktifkan untuk kontrak ini
                      </div>
                    </div>
                  </q-card>
                </div>
                ...
                <div class="row q-col-gutter-xl q-mb-xl animate-fade">
                  ...
                  <div class="col-12 col-md-4">
                    <q-card flat class="balance-prestige-card bg-white shadow-2">
                      ...
                      <q-card-section class="q-pa-xl">
                        <div
                          class="text-overline text-orange-10 text-bold uppercase tracking-widest"
                        >
                          Cost of Sales (Modal)
                        </div>
                        <div class="text-h4 text-weight-black text-dark q-mt-sm">
                  ...
                  <div class="col-12 col-md-4">
                    <q-card
                      ...
                      :class="
                        calculateGrandTotalJual(currentSpk) - calculateGrandTotalModal(currentSpk) <
                        0
...
defineOptions({
  name: 'ManufactureMasterProjectPage',
})
...
const canAction = (actionType) => {
  ...
  const menu = modulePerm.menus.find(
    (m) =>
      m.id.toLowerCase().includes('project_master') ||
      m.id.toLowerCase().includes('project-master') ||
      m.id.toLowerCase().includes('produksi/project/master'),
  )
...
const fetchProyek = () => {
  loading.value = true
  const qProyek = query(collection(db, 'mf_projects'), orderBy('createdAt', 'desc'))
  ...
  unsubAllSpk = onSnapshot(
    collection(db, 'mf_project_monitoring'),
    (snap) => {
...
const fetchSpkByProject = (pid) => {
  ...
  const q = query(collection(db, 'mf_project_monitoring'), where('projectId', '==', pid))
...
const confirmDeleteSpk = (row) => {
  ...
  $q.dialog({
    ...
    try {
      await deleteDoc(doc(db, 'mf_project_monitoring', row.id))
...
const handleSaveSpk = async () => {
  ...
  try {
    ...
    if (isEditSpkMode.value) {
      await updateDoc(doc(db, 'mf_project_monitoring', payload.id), {
        ...payload,
      ...
      await addDoc(collection(db, 'mf_project_monitoring'), payload)
    }
...
const saveRabModal = async () => {
  ...
  try {
    ...
    delete payload.id
    await updateDoc(doc(db, 'mf_project_monitoring', sid), {
      ...payload,
      updatedAt: serverTimestamp(),
    })
...
const simpanProyek = async () => {
  ...
  try {
    if (isEditMode.value) {
      await updateDoc(doc(db, 'mf_projects', p.id), p)
    } else {
      p.createdAt = serverTimestamp()
      await addDoc(collection(db, 'mf_projects'), p)
    }
...
const hapusProyek = (row) => {
  ...
  $q.dialog({
    ...
    try {
      await deleteDoc(doc(db, 'mf_projects', row.id))
...
onMounted(() => {
  ...
  ).then((c) => (optionsKonsumen.value = c.docs.map((d) => d.data().nama)))
  getDocs(collection(db, 'mf_project_categories')).then(
