              ...
              <div class="col-12 col-md-10 col-lg-8">
                ...
                >
                  ...
                  <q-card-section class="q-pa-xl">
                    <div
                      class="text-body1 text-grey-9 leading-relaxed"
            ...
            <div class="row justify-center">
              <div class="col-12 col-md-8 col-lg-6">
                <q-card
                  flat
                  bordered
                  class="rounded-20 q-pa-xl bg-white shadow-1 border-brand-thin"
                >
                  <div
                  ...
                  <div class="q-gutter-y-lg">
                    <div>
                      ...
                      <q-input
                        outlined
                        v-model="form.nama"
                        placeholder="Contoh: Project Produksi, Fabrikasi..."
                        bg-color="white"
                        autofocus
...
defineOptions({
  name: 'ManufactureKategoriProjectPage',
})
...
const canAction = (actionType) => {
  ...
  const menu = modulePerm.menus.find((m) => {
    const menuId = m.id.toLowerCase()
    return (
      menuId === '_manufaktur_project-kategori' ||
      menuId.includes('project_kategori') ||
      menuId.includes('project-kategori') ||
      menuId.includes('produksi/project/kategori')
    )
  })
...
const fetchKategori = () => {
  loading.value = true
  const q = query(collection(db, 'mf_project_categories'), orderBy('nama', 'asc'))
...
const simpanKategori = async () => {
  ...
  try {
    ...
    if (isEditMode.value) {
      await updateDoc(doc(db, 'mf_project_categories', form.value.id), payload)
    } else {
      payload.createdAt = serverTimestamp()
      await addDoc(collection(db, 'mf_project_categories'), payload)
    }
...
const hapusKategori = (data) => {
  ...
  $q.dialog({
    ...
    try {
      await deleteDoc(doc(db, 'mf_project_categories', data.id))
