      ...
      >
        ...
        <template v-slot:body="props">
          ...
          >
            ...
            <q-td key="tipe">
              <q-badge
                :color="props.row.tipe === 'ANTAR_GUDANG' ? 'green-2' : 'green-2'"
                :text-color="props.row.tipe === 'ANTAR_GUDANG' ? 'green-10' : 'green-10'"
            ...
            <div id="sj-export-target" class="perfectionist-paper">
                ...
                <div class="col-auto text-left">
                  <div class="row no-wrap items-center">
                    ...
                    <div class="column justify-center">
                      ...
                      <div class="text-pt-tagline italic text-green-9">
                        {{
              ...
              <div class="row justify-end q-mb-lg">
                <div class="col-auto text-right">
                  <div
                    class="text-h4 text-weight-black text-green-10 uppercase tracking-widest leading-none"
                  >
                    SURAT JALAN
                  </div>
                  <div class="text-subtitle1 text-weight-bold q-mt-xs">
                    No. Surat Jalan :
                    <span class="text-green-10 font-mono">{{ editFields.nomor_sj }}</span>
                  </div>
                </div>
              </div>
              ...
              <table class="perfectionist-table full-width">
                ...
                <tbody>
                  <tr v-for="(item, i) in selectedRequest.items" :key="i">
                    ...
                    <td class="text-left text-green-8">
          ...
          <div class="row justify-center no-print animate-fade">
            <div class="col-12 col-md-11 col-lg-10">
              ...
              <q-card
                flat
                bordered
                class="rounded-20 q-mb-xl bg-white shadow-premium border-green-thin overflow-hidden"
              >
                <q-card-section class="bg-green-1 q-pa-md row items-center">
                  <q-icon name="tune" color="green-10" size="sm" class="q-mr-sm" />
                  <div class="text-weight-bold text-green-10 uppercase letter-spacing-1">
                    Konfigurasi Ekspor Surat Jalan (PDF Editor)
                  </div>
                  <q-space />
                  <q-badge color="green-10" class="q-px-md q-py-xs">READY TO EXPORT</q-badge>
                </q-card-section>
              ...
              >
                ...
                <div class="col-12 col-md-2 column items-center q-py-md">
                  <q-icon name="double_arrow" size="lg" color="green-10" />
                  <q-badge
                    color="grey-3"
                    text-color="green-10"
              ...
              <div
                class="text-h6 text-weight-bold text-green-10 q-mb-md uppercase flex items-center text-left"
              >
                <q-icon name="edit_note" class="q-mr-sm" color="green-10" size="md" /> Edit
              ...
              >
                ...
                <tbody>
                  <tr v-for="(item, i) in selectedRequest.items" :key="i">
                    <td class="text-left q-pa-md font-bold text-green-10 uppercase">
                      {{ item.nama_barang }}
                    </td>
                    <td class="text-center text-h6 text-weight-black text-green-10">
                      {{ item.qty }}
                    </td>
          ...
          <template v-if="selectedRequest.status === 'Pending' || !selectedRequest.status">
            ...
            <div v-else class="row full-width flex-center">
              <q-badge
                color="green-1"
                text-color="green-10"
                class="q-pa-md border-dashed-green full-width shadow-sm"
              >
                <q-icon name="lock_person" color="green-9" class="q-mr-sm" />
...
const getStatusColor = (status) => {
  if (status === 'Approved') return 'green-10'
  if (status === 'Rejected') return 'green-10'
  if (status === 'Pending') return 'green-9'
  return 'grey-7'
}
...
const fetchRequests = () => {
  loading.value = true
  const q = query(collection(db, 'permintaan_barang_manufaktur'), orderBy('timestamp', 'desc'))
...
const fetchCurrentUser = () => {
  ...
  if (!email) return
  const q = query(collection(db, 'karyawan_manufaktur'), where('email', '==', email))
      ...
      if (status === 'Approved' && req.tipe === 'ANTAR_GUDANG') {
        await runTransaction(db, async (transaction) => {
          for (const item of req.items) {
            ...
            transaction.update(doc(db, 'stok_barang_manufaktur', sourceDoc.id), {
            ...
            if (!destSnap.empty) {
              ...
              const newStokRef = doc(collection(db, 'stok_barang_manufaktur'))
            ...
            const logOutRef = doc(collection(db, 'aktivitas_manufaktur'))
            ...
            const logInRef = doc(collection(db, 'aktivitas_manufaktur'))
          ...
          transaction.update(doc(db, 'permintaan_barang_manufaktur', req.id), {
            status: 'Approved',
        ...
        await updateDoc(doc(db, 'permintaan_barang_manufaktur', req.id), {
          status: status,
          updatedAt: serverTimestamp(),
          processedBy: userData.value?.nama || 'Administrator',
        })
...
const deleteRequest = (row) => {
  $q.dialog({
    ...
    cancel: true,
    ok: { color: 'green-10', label: 'Hapus' },
  }).onOk(async () => {
    try {
      await deleteDoc(doc(db, 'permintaan_barang_manufaktur', row.id))
...
const summaryStats = computed(() => {
  const r = rows.value || []
  return [
    { label: 'Total Request', value: r.length, icon: 'list_alt', color: 'green' },
    {
      label: 'Pending',
