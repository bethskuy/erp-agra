<template>
  <q-page class="bg-green-1 q-pa-md q-pa-md-lg font-pro">
    <!-- VIEW MODE: LIST -->
          ...
          <div class="row items-center q-col-gutter-md">
            ...
            <div class="col-12 col-md-auto row items-center q-gutter-md">
              <div class="text-caption text-grey-6 border-right q-pr-md hidden sm-block">
                Total Dokumen:
                <span class="text-weight-bold text-green-10">{{ rows.length }} Record</span>
              </div>
        ...
        >
          ...
          <template v-slot:body="props">
            ...
            >
              ...
              <q-td key="project">
                <div class="text-weight-bold text-green-9 uppercase font-12">
              ...
              <q-td key="status" class="text-center">
                <q-chip
                  :color="getStatusColor(props.row.status_workflow || props.row.status)"
                  text-color="white"
                  size="sm"
                  class="text-weight-bold shadow-sm"
                >
                  {{ getStatusLabel(props.row.status_workflow || props.row.status) }}
                </q-chip>
              </q-td>
              <q-td key="aksi" class="text-center" @click.stop>
                <div class="row justify-center q-gutter-xs">
                  ...
                  <q-btn
                    v-if="
                      canAction('ubah') &&
                      [WORKFLOW_WAITING_REVIEW, WORKFLOW_DRAFT, 'Draft', 'Rejected'].includes(
                        props.row.status_workflow || props.row.status,
                      )
                    "
                    flat
                    round
                  ...
                  <q-btn
                    v-if="
                      canAction('ubah') &&
                      ![WORKFLOW_APPROVED, 'Approved', 'Ordered'].includes(
                        props.row.status_workflow || props.row.status,
                      )
                    "
                    flat
          ...
          <div class="row justify-center">
            <div class="col-12">
              ...
              <q-card flat bordered class="rounded-12 q-mb-lg bg-white shadow-1">
                <q-card-section
                  class="bg-green-1 q-py-xs text-green-10 text-weight-bold flex items-center border-bottom"
                >
                        ...
                        <template v-slot:option="scope">
                          <q-item v-bind="scope.itemProps">
                            <q-item-section>
                              ...
                              <q-item-label caption class="text-uppercase text-green-9">{{
              ...
              <q-card flat bordered class="rounded-12 q-mb-lg bg-white shadow-1 overflow-hidden">
                <q-card-section class="bg-green-1 q-py-xs row items-center border-bottom">
                  ...
                  <div class="text-weight-bold text-green-10 uppercase font-11">
                ...
                <q-markup-table flat separator="cell" class="item-entry-table">
                  <thead>
                    <tr class="bg-green-1 text-green-10">
                      <th width="40">NO</th>
                  ...
                  <tbody>
                    <tr v-for="(item, index) in form.items" :key="index">
                      ...
                      <td class="text-right text-weight-black text-green-10 bg-green-0">
                  ...
                  <tfoot class="bg-grey-1">
                    <tr>
                      ...
                      <td class="text-right text-weight-bolder text-green-10">
                        Rp {{ calculateTotalPR().toLocaleString() }}
                      </td>
                      <td></td>
                    </tr>
                    <tr class="bg-green-10 text-white">
                      <td
                        colspan="5"
              ...
              <div class="row q-col-gutter-md">
                <div class="col-12 col-md-6">
                  <q-card flat bordered class="rounded-12 bg-white shadow-1 h-full">
                    <q-card-section
                      class="bg-grey-1 q-py-xs text-green-10 text-weight-bold uppercase font-8 border-bottom"
                    >
                  ...
                  <q-card flat bordered class="rounded-12 bg-white shadow-1">
                    ...
                    <q-card-section class="q-pa-md">
                      ...
                      <div class="row q-col-gutter-lg items-end">
                        ...
                        <div class="col-12 col-sm-6">
                          <div
                            class="text-caption text-bold text-green-10 q-mb-xs uppercase font-10"
                          >
                        ...
                        <div class="col-12 col-sm-6">
                          <div
                            class="text-caption text-bold text-green-10 q-mb-xs uppercase font-8 flex items-center justify-between"
                          >
    ...
    <q-dialog v-model="showPad" persistent backdrop-filter="blur(4px)">
      <q-card style="width: 500px; max-width: 95vw" class="rounded-20 shadow-24">
        <q-card-section class="row items-center q-pb-none bg-green-10 text-white q-pa-md">
    ...
    <q-dialog v-model="showPreview" maximized transition-show="fade" transition-hide="fade">
      <q-card class="column no-wrap bg-grey-4">
        <q-toolbar class="bg-white text-green-10 q-py-md no-print shadow-2 shrink">
            ...
            <table class="final-pro-table full-width">
              ...
              <tfoot>
                <tr class="row-calculation">
                  ...
                  <td class="text-right text-bold text-green-10">
            ...
            <div class="signature-container text-left q-mt-xl">
              ...
              <div class="row q-mt-lg justify-end">
                <div class="col-5 text-center">
                  ...
                  <div
                    class="text-signer-final text-weight-black underline uppercase text-green-10"
                  >
                    {{
        ...
        <table class="report-table">
          ...
          <tbody>
            <tr v-for="(row, idx) in rows" :key="row.id">
              ...
              <td style="font-weight: 900; color: #1b5e20">{{ row.nomor }}</td>
...
let unsubUser = null
let unsubRows = null
const WORKFLOW_WAITING_REVIEW = 'waiting_pr_review'
...
const normalizePrRow = (row) => ({
  ...row,
  status: row.status || WORKFLOW_DRAFT,
  status_workflow: row.status_workflow || row.workflow_status || row.status || WORKFLOW_DRAFT,
  items:
    Array.isArray(row.items) && row.items.length
      ? row.items.map((item) => {
          const namaBarang =
            item.nama_barang || item.deskripsi || item.material || item.barang?.nama || ''
          const satuan = item.satuan || item.unit || item.barang?.unit || 'ls'
          const estimasiHarga = Number(item.estimasi_harga || item.est_harga || item.harga || 0)
          const qty = Number(item.qty || item.jumlah || item.qty_kebutuhan || 1)
          return {
            ...item,
            barang:
              item.barang && typeof item.barang === 'object'
                ? item.barang
                : { id: item.id_barang || item.material_id || '', nama: namaBarang, unit: satuan },
            id_barang: item.id_barang || item.material_id || '',
            nama_barang: namaBarang,
            qty,
            satuan,
            estimasi_harga: estimasiHarga,
            supplier_id: item.supplier_id || '',
            supplier: item.supplier || item.supplier_nama || '',
            note: item.note || item.catatan || item.keterangan || '',
            total: Number(item.total || qty * estimasiHarga || 0),
          }
        })
      : JSON.parse(JSON.stringify(formDefault.items)),
})

const fetchData = async () => {
  loading.value = true
  const confRef = doc(db, 'config_manufaktur', 'perusahaan')
  const configSnap = await getDoc(confRef)
  if (configSnap.exists()) config.value = configSnap.data()

  const unreadNotificationSnap = await getDocs(
    query(
      collection(db, GUDANG_NOTIFICATION_COLLECTION),
      where('type', '==', 'PR_BARU_DARI_PPIC'),
      where('is_read_gudang', '==', 0),
    ),
  )
  unreadNotificationSnap.docs.forEach((notificationDoc) => {
    updateDoc(doc(db, GUDANG_NOTIFICATION_COLLECTION, notificationDoc.id), {
      is_read_gudang: 1,
      read_gudang_at: serverTimestamp(),
      updated_at: serverTimestamp(),
    }).catch((err) => console.error('Gagal update notifikasi PR Baru PPIC:', err))
  })

  // FETCH ARRAY (Filter jika masuk dari Gudang/Project tertentu)
  unsubRows = onSnapshot(collection(db, 'manufactur_purchase_request'), (snap) => {
    const all = snap.docs.map((d) => normalizePrRow({ id: d.id, ...d.data() }))
    all
      .filter((it) => it.status_workflow === WORKFLOW_WAITING_REVIEW && it.is_read_gudang === 0)
      .forEach((it) => {
        updateDoc(doc(db, 'manufactur_purchase_request', it.id), {
          is_read_gudang: 1,
          read_gudang_at: serverTimestamp(),
        }).catch((err) => console.error('Gagal update notifikasi PR Gudang:', err))
      })
    rows.value = all
  ...
  const barSnap = await getDocs(collection(db, 'manufactur_master_barang'))
  allBarang.value = barSnap.docs
    .map((d) => {
      const data = d.data()
      const vendorPrices = Array.isArray(data.vendor_prices) ? data.vendor_prices : []
      const bestVendor = vendorPrices
        .map((item) => ({
      ...
      return {
        id: d.id,
        nama: data.nama || data.nama_barang || data.nama_material || '',
        unit: data.unit || data.satuan || data.uom || '',
        supplier_id: data.supplier_id || data.supplier_default_id || bestVendor?.id || '',
        supplier: data.supplier || data.supplier_nama || data.supplier_default || bestVendor?.nama || '',
        estimasi_harga: Number(data.harga_terendah || data.estimasi_harga || bestVendor?.harga || 0),
      }
    })
  ...
  try {
    const spkSnap = await getDocs(collection(db, 'spk_customer_manufaktur'))
...
const openEditDialog = (row) => {
  ...
  form.value = JSON.parse(JSON.stringify(normalizePrRow(row)))
...
const ajukanPR = (row) => {
  if (!row.signatureUrl) {
    $q.notify({
      type: 'warning',
      message: 'Tanda tangan Gudang wajib diisi sebelum submit approval.',
    })
    openEditDialog(row)
    return
  }
  $q.dialog({
    ...
    cancel: true,
    ok: { color: 'green-10', unelevated: true, label: 'Ya, Kirim' },
  }).onOk(async () => {
    try {
      await updateDoc(doc(db, 'manufactur_purchase_request', row.id), {
        status: WORKFLOW_SUBMITTED,
        status_workflow: WORKFLOW_SUBMITTED,
        workflow_status: WORKFLOW_SUBMITTED,
        gudang_status: WORKFLOW_SUBMITTED,
        prepared_by: userData.value?.nama || authStore.user?.nama || authStore.user?.email || 'Gudang',
...
const openPreview = async (r) => {
  ...
  if ((r.status === 'Approved' || r.status === 'Rejected') && r.requester_read === false) {
    try {
      await updateDoc(doc(db, 'manufactur_purchase_request', r.id), { requester_read: true })
...
const submitPurchaseRequest = async () => {
  ...
  try {
    const currentWorkflow = form.value.status_workflow || form.value.workflow_status || form.value.status
    const nextWorkflow =
      currentWorkflow === WORKFLOW_WAITING_REVIEW ? WORKFLOW_DRAFT : currentWorkflow || WORKFLOW_DRAFT
    const payload = {
      ...form.value,
      ...
      status: form.value.status === WORKFLOW_WAITING_REVIEW ? WORKFLOW_DRAFT : form.value.status || WORKFLOW_DRAFT,
      status_workflow: nextWorkflow,
      workflow_status: nextWorkflow,
      gudang_status: nextWorkflow,
      pemohon: {
    ...
    payload.items = payload.items.map((it) => {
      ...
      return {
        ...
        note: it.note || '',
        total: Number(it.total) || Number(it.qty || 0) * Number(it.estimasi_harga || it.est_harga || 0),
      }
    })
    ...
    if (isEditMode.value && docId) {
      delete payload.id
      await updateDoc(doc(db, 'manufactur_purchase_request', docId), payload)
    } else {
      payload.timestamp = serverTimestamp()
      payload.status = WORKFLOW_DRAFT
      payload.status_workflow = WORKFLOW_DRAFT
      payload.workflow_status = WORKFLOW_DRAFT
      payload.gudang_status = WORKFLOW_DRAFT
      await addDoc(collection(db, 'manufactur_purchase_request'), payload)
    }
...
const handleLogoChange = async (f) => {
  ...
  resizeImageToBase64(f, 400).then((res) => {
    ...
    setDoc(doc(db, 'config_manufaktur', 'perusahaan'), { kopUrl: res }, { merge: true })
  })
}
...
const onBarangSelect = (v, idx) => {
  if (v) {
    if (typeof v === 'string') {
      ...
      form.value.items[idx].supplier_id = v.supplier_id || ''
      form.value.items[idx].supplier = v.supplier || ''
      form.value.items[idx].estimasi_harga = Number(v.estimasi_harga || 0)
    }
    calcRow(idx)
  }
}
...
const getStatusColor = (s) =>
  s === 'Approved' || s === WORKFLOW_APPROVED
    ? 'positive'
    : s === 'Rejected' || s === WORKFLOW_REJECTED
      ? 'negative'
      : s === 'Ordered'
        ? 'green-10'
        : s === 'Pending' || s === WORKFLOW_SUBMITTED || s === WORKFLOW_WAITING_REVIEW
          ? 'orange-8'
          : 'green-7'

const getStatusLabel = (s) => {
  const labels = {
    [WORKFLOW_WAITING_REVIEW]: 'Waiting PR Review',
...
const exportListToExcel = () => {
  ...
  let html = `
    ...
    <style>
      ...
      .header-row th { background-color: #1b5e20; color: #ffffff; font-weight: bold; text-align: left; }
      .title { font-size: 18px; font-weight: bold; color: #1b5e20; font-family: sans-serif; }
  ...
  rows.value.forEach((row, idx) => {
    ...
    html += `
      <tr>
        ...
        <td style="font-weight: bold; color: #1b5e20;">${row.nomor}</td>
  ...
  html += `
        <tr>
          ...
          <td align="right" style="font-weight: bold; color: #1b5e20; font-size: 14px; background-color: #f5f5f5;">${totalAmount}</td>
...
const confirmHapus = (r) => {
  $q.dialog({ title: 'Hapus PR?', cancel: true }).onOk(async () => {
    await deleteDoc(doc(db, 'manufactur_purchase_request', r.id))
  })
}
