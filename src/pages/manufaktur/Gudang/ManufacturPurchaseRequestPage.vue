        ...
        >
          ...
          <template v-slot:body="props">
            ...
            >
              ...
              <q-td key="gudang">
                <div class="text-weight-bold text-green-9 uppercase">
                ...
                <div
                  ...
                  class="text-green-10 text-caption italic"
                >
    ...
    <q-dialog v-model="showPad" persistent backdrop-filter="blur(4px)">
      <q-card style="width: 500px; max-width: 95vw" class="rounded-20 shadow-24">
        <q-card-section class="row items-center q-pb-none bg-green-10 text-white q-pa-md">
          <div class="text-h6 text-weight-bold uppercase font-10">Tanda Tangan Prepagreen By</div>
          <q-space />
      ...
      <q-card class="column no-wrap bg-grey-4">
        <q-toolbar class="bg-white text-green-10 q-py-md no-print shadow-2 shrink">
          ...
          <q-btn-group unelevated rounded class="shadow-2">
            <q-btn color="green-10" icon="print" label="Cetak" @click="printPage" class="q-px-md" />
            <q-btn color="green-9" icon="picture_as_pdf" label="PDF" @click="exportToPDF" />
          </q-btn-group>
        </q-toolbar>
            ...
            <table class="final-pro-table full-width">
              ...
              <tbody>
                <tr v-for="(it, i) in selectedData.items" :key="i">
                  ...
                  <td class="text-right text-weight-bolder text-green-10 bg-green-0">
...
const fetchData = async () => {
  ...
  unsubRows = onSnapshot(collection(db, 'permintaan_barang_manufaktur'), (snap) => {
  ...
  const barSnap = await getDocs(collection(db, 'master_barang_manufaktur'))
...
const ajukanPR = (row) => {
  $q.dialog({
    ...
    cancel: true,
    ok: { color: 'green-9', unelevated: true, label: 'Ya, Kirim' },
  }).onOk(async () => {
    try {
...
const handleLogoChange = async (f) => {
  ...
  r.onload = async () => {
    ...
    await setDoc(doc(db, 'config_manufaktur', 'perusahaan'), { kopUrl: r.result }, { merge: true })
  }
}
const calcRow = (idx) => {
...
const confirmHapus = (r) => {
  $q.dialog({ title: 'Hapus PR?', cancel: true }).onOk(async () => {
    await deleteDoc(doc(db, 'permintaan_barang_manufaktur', r.id))
  })
}
