<template>
  <q-page class="bg-grey-2 q-pa-md q-pa-md-lg font-pro">
    ...
    <div class="row items-center justify-between q-mb-xl animate-fade no-print">
      <div class="col-12 col-md-8">
        <div class="row items-center no-wrap">
          ...
          <div>
            <div
              class="text-h4 text-weight-bolder text-green-10 leading-tight uppercase tracking-widest"
            >
              Riwayat Transaksi
      ...
      <q-card-section class="q-py-md">
        <div class="row items-center q-col-gutter-md">
          ...
          <div class="col-12 col-md-auto text-right">
            ...
            >
              {{ filtegreenRows.length }} RECORD LOKASI INI
            </q-badge>
          </div>
        </div>
      </q-card-section>
    ...
    <q-card
      flat
      bordered
      class="rounded-20 shadow-premium overflow-hidden bg-white no-print border-green-thin"
    >
      <q-table
        :rows="filtegreenRows"
        :columns="columns"
    ...
    >
      <q-card class="column no-wrap overflow-hidden bg-grey-2" v-if="selectedItem">
            ...
            <div class="col-12 col-md-11 col-lg-10">
              ...
              <div class="row justify-between items-end q-mb-xl animate-fade">
                ...
                <div class="text-right">
                  ...
                  <div class="text-h4 text-weight-black text-green-10">
                    {{ formatDate(selectedItem.timestamp) }}
                  </div>
                  <div class="text-subtitle1 text-green-10 text-weight-bold">
              ...
              <q-card
                flat
                bordered
                class="rounded-20 bg-white shadow-premium border-green-thin overflow-hidden q-mb-xl"
              >
                <q-card-section
                  class="bg-green-1 q-pa-md text-weight-black text-green-10 uppercase tracking-widest flex items-center"
                >
                  <q-icon name="folder_shagreen" class="q-mr-sm" /> Rincian Administrasi
                </q-card-section>
                <q-card-section class="q-pa-lg">
                  <div class="row q-col-gutter-xl text-left">
                    <div class="col-12 col-sm-4">
                      ...
                      <div class="text-h6 text-weight-black text-green-10">
                    ...
                    <div class="col-12 col-sm-4 border-left-gt-xs">
                      ...
                      <div class="text-h6 text-weight-bold text-green-10 uppercase">
              ...
              <div
                class="text-h6 text-weight-black text-green-10 uppercase q-mb-md flex items-center letter-spacing-1"
              >
                <q-icon name="list_alt" class="q-mr-sm" color="green-10" />
              ...
              >
                <q-markup-table flat separator="cell" class="perfectionist-table">
                  <thead>
                    <tr class="bg-green-10 text-white text-bold uppercase font-11">
                      <th width="60">NO</th>
                  ...
                  <tbody class="text-green-10">
                    <tr v-for="(it, i) in groupedItems" :key="i">
                      ...
                      <td class="italic text-weight-bolder text-green-10 bg-green-1">
                        {{ it.keterangan || '-' }}
                      </td>
                    </tr>
                  </tbody>
              ...
              <div class="q-mb-xl">
                <q-card flat bordered class="rounded-20 bg-white shadow-sm border-subtle">
                  <q-card-section
                    class="bg-grey-1 q-pa-sm text-weight-bold text-green-8 uppercase text-caption q-px-md tracking-widest flex items-center"
                  >
                    <q-icon name="comment" class="q-mr-xs" /> Catatan Umum / Instruksi Transaksi
                  </q-card-section>
                  <q-card-section class="q-pa-lg text-left">
                    <div class="text-body1 text-green-9 italic leading-relaxed">
                      "{{
              ...
              <div class="q-mb-xl" v-if="selectedItem.dokumentasi_urls?.length">
                <div
                  class="text-h6 text-green-10 text-weight-black uppercase q-mb-md flex items-center letter-spacing-1"
                >
                    ...
                    >
                      <q-card-section class="q-pa-sm">
                        ...
                        <div
                          ...
                          style="height: 200px"
                        >
                          <q-icon name="description" size="80px" color="green-4" />
                        </div>
                        <div
                          class="text-center text-weight-bold text-green-9 q-mt-sm ellipsis q-px-md"
                        >
        ...
        >
          Agra ERP Logistics • Internal Transaction Audit Log • Secugreen Document
        </div>
      </q-card>
    </q-dialog>
    ...
    <div style="position: absolute; left: -9999px; top: -9999px">
      <div id="sj-reprint-target" class="perfectionist-paper">
        ...
        <div class="row justify-end q-mb-lg" v-if="selectedItem">
          <div class="col-auto text-right">
            ...
            <div class="text-subtitle1 text-weight-bold q-mt-xs">
              No. Surat Jalan :
              <span class="text-green-10 font-mono">{{ selectedItem.no_referensi }}</span>
            </div>
          </div>
        </div>
        ...
        <table class="perfectionist-table full-width" v-if="selectedItem">
          ...
          <tbody>
            <tr v-for="(it, i) in groupedItems" :key="i">
              ...
              <td class="text-left text-green-8 italic">{{ it.keterangan || '-' }}</td>
            </tr>
          </tbody>
          <tfoot>
            <tr class="bg-grey-1">
        ...
        <div class="q-mt-xl text-center text-grey-5 italic" style="font-size: 9.5px">
          Secugreen Digital Archive • Reprinded via Agra ERP v4.1.2 • Original Hash Verified
        </div>
      </div>
    </div>
...
const getTipeColor = (t) => (t === 'MASUK' ? 'green-10' : t === 'KELUAR' ? 'green-8' : 'green-8')
...
const openDetail = async (row) => {
  ...
  if (row.no_referensi && row.no_referensi !== '-') {
    ...
    try {
      ...
      const qGroup = query(
        collection(db, 'aktivitas_manufaktur'),
...
const refreshListener = async () => {
  ...
  if (warehouseId.value === 'UTAMA') {
    ...
    const pSnap = await getDoc(doc(db, 'proyek_manufaktur', warehouseId.value))
  ...
  const q = query(collection(db, 'aktivitas_manufaktur'), orderBy('timestamp', 'desc'))
